...
-- ── 3b. RPC: per-user weakness profile ─────────────────────────────────────
-- Canonical definition: rag_schema.sql § get_user_weakness_profile.
-- Quick prod fix if missing: supabase/patch_coach_weakness.sql
-- (coach_schema.sql intentionally does not redefine this RPC to avoid drift.)
