...
function refundStatusLabel(refundReview, autoRefund) {
  if (!refundReview) return 'Not requested'
  if (autoRefund?.issued) {
    const amt = `${(autoRefund.netCents / 100).toFixed(2)} ${String(autoRefund.currency || 'usd').toUpperCase()}`
    return `AUTO-REFUNDED ${amt}`
  }
  return 'Requested — needs your review'
}

function refundNote(refundReview, autoRefund) {
  if (!refundReview) return ''
  if (autoRefund?.issued) {
    const fee = autoRefund.feeCents != null ? ` Stripe's $${(autoRefund.feeCents / 100).toFixed(2)} fee was deducted.` : ''
    return `<p style="margin-top:18px"><strong>Refund note:</strong> Issued automatically (inside the ${autoRefund.windowDays}-day window). Subscription cancelled immediately and premium revoked. Nothing to do.${fee}</p>`
  }
  const why = autoRefund?.reason ? ` Auto-refund did not apply: <code>${escapeHtml(autoRefund.reason)}</code>${autoRefund.ageDays != null ? ` (payment is ${autoRefund.ageDays.toFixed(1)} days old, window is ${autoRefund.windowDays}d)` : ''}.` : ''
  return `<p style="margin-top:18px"><strong>Refund note:</strong> The user requested a refund review.${why} If approving, use the Refund button in Admin → Billing; it refunds to the original payment method minus Stripe's fee and ends access immediately.</p>`
}

export function renderCancellationFeedbackNotice({ name, email, userId, plan, periodEnd, reason, feedback, wouldReturn, refundReview, autoRefund = null, stripeSubscriptionId }) {
  ...
  const body = `<p>A user scheduled a subscription cancellation and submitted feedback.</p>
    <table cellspacing="0" cellpadding="0" style="width:100%;font-size:14px;margin-top:8px">
      ...
      <tr><td style="color:#64748b;padding:6px 0">Refund</td><td style="padding:6px 0;text-align:right;font-weight:600">${refundStatusLabel(refundReview, autoRefund)}</td></tr>
    ...
    ${refundNote(refundReview, autoRefund)}`
