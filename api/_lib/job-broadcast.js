...
export default async function handler(req, res) {
  ...
  if (!dryRun && due.status === 'scheduled') {
    await tryDb(supabase.from('broadcasts').update({ status: 'sending', updated_at: nowIso }).eq('id', due.id), 'broadcasts.status')
  }
    ...
    for (const profile of rows) {
      ...
      if (!dryRun) {
        ...
        if (sendResult.ok) {
          ...
          if (sendResult.logId) {
            await tryDb(
              supabase.from('marketing_sends').update({ email_log_id: sendResult.logId })
                .eq('user_id', profile.id).eq('campaign_key', campaignKey),
              'marketing_sends.log_id'
            )
          }
        } else {
          result.failed++
          await tryDb(
            supabase.from('marketing_sends').delete()
              .eq('user_id', profile.id).eq('campaign_key', campaignKey),
            'marketing_sends.rollback'
          )
        }
      } else {
        result.sent++
  ...
  if (!dryRun) {
    ...
    await tryDb(supabase.from('broadcasts').update(patch).eq('id', due.id), 'broadcasts.status')
  }
