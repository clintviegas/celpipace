...
import { checkFreeEvalQuota } from './quota.js';
...
export default async function handler(req, res) {
  ...
  // Enforced fail-closed — see api/_lib/quota.js for why.
  const FREE_SPEAKING_EVALS = 2;
  const quota = await checkFreeEvalQuota({
    supabase: auth.supabase,
    userId,
    section: 'speaking',
    limit: FREE_SPEAKING_EVALS,
  });
  if (!quota.allowed) return res.status(quota.status).json(quota.body);
