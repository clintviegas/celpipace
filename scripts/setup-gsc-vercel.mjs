...
if (slackWebhook) {
  if (!slackWebhook.startsWith('https://hooks.slack.com/')) {
    console.error('SLACK_WEBHOOK_URL must be a Slack Incoming Webhook URL.')
    process.exit(1)
  }
  addEnv('SLACK_WEBHOOK_URL', slackWebhook)
} else {
  console.log('\nTip: pass SLACK_WEBHOOK_URL=... to also push the Slack webhook for #all-celpipace')
}

console.log('\nDone. Redeploy production for /api/cron?job=gsc-summary to go live.')
console.log('Cron:      Mondays 05:00 UTC (09:00 GST) via vercel.json')
console.log('Manual:    GET https://www.celpipace.ca/api/cron?job=gsc-summary')
