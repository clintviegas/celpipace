...
import { AUTH_CONSENT_STORAGE_KEY, PRODUCT_STATS, TERMS_VERSION } from '../data/constants'
...
export default function AuthModal({ isOpen, onClose, reason }) {
  ...
  const handleGoogle = async () => {
    ...
    setConsentError('')
    if (!termsAccepted) {
      setConsentError('Please accept the Terms & Conditions before continuing.')
      return
    }
    setSending(true)
    try {
      window.localStorage.setItem(AUTH_CONSENT_STORAGE_KEY, JSON.stringify({
        termsAccepted: true,
      ...
      // OAuth can continue even if localStorage is unavailable.
    }
              ...
              <AnimatePresence mode="wait">
                ...
                <Motion.div key="google" className="auth-form-wrap"
                  ...
                  transition={{ duration: 0.22 }}>
                    ...
                    <div className="auth-consent-options">
                      <label className="auth-consent-row auth-consent-row--required">
                        <input
                          type="checkbox"
                        ...
                        />
                        <span>
                          I agree to the <a className="auth-link" href="/terms">Terms &amp; Conditions</a>, <a className="auth-link" href="/privacy">Privacy Policy</a>, and <a className="auth-link" href="/refund">Refund Policy</a>.
                        </span>
                      </label>
                      <label className="auth-consent-row">
                        ...
                        />
                        <span>Send me CELPIP study tips, mock-test reminders, and product updates by email. I can unsubscribe anytime.</span>
                      </label>
                    </div>

                    {consentError && <div className="auth-email-error auth-consent-error">{consentError}</div>}

                    <button className="auth-btn auth-btn--google auth-btn--google-main" onClick={handleGoogle} disabled={sending || !termsAccepted}>
                      ...
                      {sending ? 'Opening Google...' : termsAccepted ? 'Continue with Google' : 'Accept terms to continue'}
                    </button>
                    ...
                    <p className="auth-modal-legal">
                      Email preferences can be changed anytime from your subscription page.
                    </p>
