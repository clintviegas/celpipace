...
export function trackSignupComplete(userId) {
  if (!userId) return
  trackAnalyticsEvent('signup_complete', { userId })
}

export function trackCheckoutStarted(userId, metadata = {}) {
  trackAnalyticsEvent('checkout_started', { userId, metadata })
}

export function trackCheckoutError(userId, metadata = {}) {
  trackAnalyticsEvent('checkout_error', { userId, metadata })
}

export function trackUpgradeClick(userId, metadata = {}) {
  trackAnalyticsEvent('upgrade_click', {
    userId,
    elementLabel: metadata.label || 'upgrade',
    metadata,
  })
}
...
export function initClickAnalytics(getUserId) {
  ...
  const onClick = (event) => {
    ...
    const analyticsTag = element.getAttribute('data-analytics')
    if (analyticsTag?.startsWith('upgrade')) {
      trackUpgradeClick(getUserId?.() || null, { label: analyticsTag, source: window.location.pathname })
    }
    trackAnalyticsEvent('click', {
      ...
      metadata: {
        ...
        data_analytics: analyticsTag || null,
      },
    })
  }
