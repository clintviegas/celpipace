...
async function maybeSyncNewUser(profile) {
  ...
  try {
    ...
    if (isNew) {
      sessionStorage.setItem(signupFlag, '1')
      trackSignupComplete(profile.id)
    }
  } catch (error) {
...
export function AuthProvider({ children }) {
  ...
  const touchUserActivity = useCallback(async (currentUser) => {
    ...
    try {
      ...
      if (!error) return
      // Fallback when the RPC is unavailable. A query builder returns
      // { error } rather than rejecting, and has no .catch() of its own —
      // the enclosing try/catch covers transport failures.
      await supabase
        .from('profiles')
  ...
  const loadProfile = useCallback(async (currentUser) => {
    ...
    try {
      ...
      if (!data) {
        ...
        // No .catch() here — a query builder resolves to { error } and has no
        // catch(). The previous version threw a TypeError on this line, which
        // the outer try/catch turned into setProfile(null), so this fallback
        // never actually created a row for users the DB trigger missed.
        ...
        if (upsertErr) console.warn('[auth] profile bootstrap upsert failed:', upsertErr.message)
      ...
      // A query builder resolves to { error } rather than rejecting and has no
      // .catch() of its own; calling one here threw a TypeError inside a .then()
      // with no rejection handler, so this backfill never ran and surfaced as an
      ...
      if (!data.country_code) {
        fetchGeo()
          .then(async geo => {
            ...
            if (error) console.warn('[auth] geo backfill failed:', error.message)
          })
          .catch(err => console.warn('[auth] geo backfill threw:', err?.message))
      }
    } catch (e) {
