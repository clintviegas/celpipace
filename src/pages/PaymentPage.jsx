...
import { trackCheckoutError, trackCheckoutStarted } from '../lib/analytics'
...
export default function PaymentPage({ onSignIn }) {
  ...
  const startCheckout = async () => {
    ...
    setMessageType('info')
    try {
      trackCheckoutStarted(user.id, { plan: selected, coupon: activeCoupon?.code || null })
      ...
      trackCheckoutError(user.id, { plan: selected, message: error.message || 'checkout_failed' })
      setMessageType('error')
