...
export default function ManageSubscriptionPage() {
  ...
  // Settle the subscription on return from checkout.
  //
  // This used to hope the webhook had landed and blind-refresh twice. Now we
  // ask the server to read the Checkout Session straight from Stripe and apply
  // the same patch the worker would, so premium is active by the time the page
  // settles — no "I paid but I'm still on free" window if webhook delivery is
  // slow or fails. The timed refreshes stay as a fallback for the case where
  // Stripe has not finished creating the subscription yet.
  useEffect(() => {
    ...
    const settle = async () => {
      if (sessionId) {
        try {
          ...
          if (cancelled) return
          if (res.ok && data.settled) { await refreshProfile?.(); return }
        } catch { /* fall through to the refresh fallback */ }
      }
      if (cancelled) return
      timers.push(setTimeout(() => refreshProfile?.(), 1500))
      timers.push(setTimeout(() => refreshProfile?.(), 5000))
    }
    ...
    >
      ...
      >
        ...
        >
          ...
          />
          <span>
            Request a refund. If you're within your plan's money-back window it's issued instantly (minus the card processing fee) and your access ends now; otherwise it goes to support for review. Refunds usually appear within 3-4 business days.
          </span>
        </label>
...
function EmailPreferencesCard({ initialConsent, onChanged }) {
  ...
  const toggle = async (next) => {
    ...
    try {
      const res = await authedFetch('/api/on-signup', { body: { consent: next } })
      const data = await res.json().catch(() => ({}))
