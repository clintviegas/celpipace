...
const HomeDesktopSections = lazyWithRetry(() => import('./components/HomeDesktopSections'))
...
const TipsPage = lazyWithRetry(() => import('./pages/TipsPage'))
const ScoresPage = lazyWithRetry(() => import('./pages/ScoresPage'))
const CRSCalculatorPage = lazyWithRetry(() => import('./pages/CRSCalculatorPage'))
const ExamPage = lazyWithRetry(() => import('./pages/ExamPage'))
const ListeningPage = lazyWithRetry(() => import('./pages/ListeningPage'))
const ReadingPage = lazyWithRetry(() => import('./pages/ReadingPage'))
const ReadingPracticePage = lazyWithRetry(() => import('./pages/ReadingPracticePage'))
const WritingPage = lazyWithRetry(() => import('./pages/WritingPage'))
const SpeakingPage = lazyWithRetry(() => import('./pages/SpeakingPage'))
const PracticeSetPage = lazyWithRetry(() => import('./pages/PracticeSetPage'))
const DashboardPage = lazyWithRetry(() => import('./pages/DashboardPage'))
const CoachPage = lazyWithRetry(() => import('./pages/CoachPage'))
const ReviewPage = lazyWithRetry(() => import('./pages/ReviewPage'))
const StudyPlanPage = lazyWithRetry(() => import('./pages/StudyPlanPage'))
...
const PredictionPage = lazyWithRetry(() => import('./pages/PredictionPage'))
const BlogPage = lazyWithRetry(() => import('./pages/BlogPage'))
const WritingPracticePage = lazyWithRetry(() => import('./pages/WritingPracticePage'))
const MockTestPage = lazyWithRetry(() => import('./pages/MockTestPage'))
const AdminPage = lazyWithRetry(() => import('./pages/AdminPage'))
const CancelFeedbackPage = lazyWithRetry(() => import('./pages/CancelFeedbackPage'))
const ManageSubscriptionPage = lazyWithRetry(() => import('./pages/ManageSubscriptionPage'))
...
const MOBILE_HOME_SECTIONS = [
  { ...SECTION_LIBRARY.speaking, Icon: Mic, sub: `8 tasks · AI-scored` },
    ...
    <section className="mh-app" aria-label="Mobile home">
      <div className="mh-hero-card">
        ...
        <div className="mh-actions">
          <button className="mh-primary" onClick={() => navigate('/celpip-speaking-practice/S1')}>Try a speaking task</button>
          <button className="mh-secondary" onClick={() => navigate('/study-coach')}>AI Study Coach</button>
        </div>
      </div>
...
export function AppInner() {
  ...
  // Emailed to churned users — a focused single-purpose page, so no site chrome.
  ...
  // Admin and /feedback are self-contained — no site chrome
  if (isAdminRoute || isFeedbackRoute) {
    return (
      <ErrorBoundary>
        <Suspense fallback={<RouteLoader />}>
          <Routes>
            <Route path="/admin" element={<AdminPage />} />
            <Route path="/feedback" element={<CancelFeedbackPage />} />
          </Routes>
        </Suspense>
      </ErrorBoundary>
    )
